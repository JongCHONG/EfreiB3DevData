...
public class Helpers {
	...
	public static void playerAction(Job job, int PlayerChoice, Monster Monster, Character Player1, int playerAttackDamage, int monsterAttackDamage, Weapon Player1Weapon) {
		int damage;
		if(PlayerChoice == 1) { 
			Texts.playerAttackMonster(Player1, Monster, playerAttackDamage, Player1Weapon);
		...
		if(PlayerChoice == 2) { 
			if (job.getJobName() == JobNames.WARRIOR.getJobName()) {
				...
				Texts.warriorUseGuard(Monster, Player1, playerAttackDamage);

			}
			if (job.getJobName() == JobNames.SORCERER.getJobName()) {
				playerAttackDamage = playerAttackDamage + job.firebolt();
				Texts.sorcererUseFirebolt(Player1, Monster, playerAttackDamage);
				damage = Helpers.healthDamage(playerAttackDamage, Monster.getMonsterHealth());
				Monster.setMonsterHealth(damage);
			...
			if (job.getJobName() == JobNames.THIEF.getJobName()) {
				...
				int lifeSteal = playerAttackDamage / job.stealLife();
				Texts.thiefUseStealLife(Monster, Player1, playerAttackDamage, lifeSteal);
				damage = Helpers.healthDamage(playerAttackDamage, Monster.getMonsterHealth());
				Player1.setHealthPoints(Player1.getHealthPoints() + lifeSteal);
				Monster.setMonsterHealth(damage);
			}
		}
	}
